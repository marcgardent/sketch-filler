...
BACKEND_REDIST_FILES = ["BackendShared.dll", "BackendShared.lib", "BackendShared.qmltypes", "BackendSharedplugin.dll",
                        "BackendSharedplugin.lib", "qmldir"]
BACKEND_SOURCE_FILES = ["renderer.h", "renderer.cpp"]
...
def reset_cmake(hard_reset=True):
    if hard_reset:
        try:
            ...
            pass
    mandatory_exec(
        [CMAKE, "-DCMAKE_BUILD_TYPE=Release", f"-DCMAKE_MAKE_PROGRAM={NINJA}", "-G", "Ninja", "-S", SOURCE_DIR, "-B",
         BINARY_DIR])


def sync_shared_backend():
    for file in BACKEND_SOURCE_FILES:
